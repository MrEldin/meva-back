...
namespace App\Console\Meva\Generators;

use App\Console\Meva\Generator;
use Prettus\Repository\Generators\Migrations\SchemaParser;
use Prettus\Repository\Generators\ValidatorGenerator;

class GenerateMevaController extends Generator
{
    /**
     * Get stub name.
     *
    ...
    {
        $contract = (new GenerateMevaContract(['name' => $this->name]))->getMainNamespace('Repository');
        $transformer = (new GenerateMevaTransformer(['name' => $this->name]))->getMainNamespace('Transformer');

        $createRequest = (new GenerateMevaRequest([
            'name' => $this->name,
            'sub_name' => 'Create',
        ]))->getMainNamespace('Request');

        $updateRequest = (new GenerateMevaRequest([
            'name' => $this->name,
            'sub_name' => 'Update',
        ]))->getMainNamespace('Request');

        $getRequest = (new GenerateMevaRequest([
            'name' => $this->name,
            'sub_name' => 'Get',
        ]))->getMainNamespace('Request');

        $deleteRequest = (new GenerateMevaRequest([
            'name' => $this->name,
            'sub_name' => 'Delete',
        ]))->getMainNamespace('Request');
