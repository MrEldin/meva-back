...
use Meva\AI\Agents\EntityClassificationAgent;
use Meva\AI\Agents\EntityQuestionAgent;
use Meva\AI\Agents\EntitySummaryAgent;
use Meva\AI\EntityIntelligence;
use Meva\AI\Facades\Intelligence;
use Meva\AI\Tools\EntityLookupTool;
...
it('answers a question that is not about any entity', function () {
    Ai::fakeAgent(AnonymousAgent::class, ['Meva is a Laravel API boilerplate.']);

    $response = Intelligence::ask('What is Meva?');

    expect($response->text)->toBe('Meva is a Laravel API boilerplate.');
});
